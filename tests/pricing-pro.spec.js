/* SPEC from docs: 
● Pro - Base: 1 User, 1,000 Tracks - $25 PCM
...
test(`PRO pricing shows $${monthlyPrice} & "${checkPricingStr}`, async ({ page }) => {  
  // Navigate to the pricing page using the base URL from .env  
  await page.goto(`${process.env.BASE_URL}/pricing`);

  // Select the PRO section by finding a section containing the "Pro" title
  const pageSection = page.locator('.PricingPlan:has(.PricingPlan_Title.heading--H4:has-text("Pro"))');
  await pageSection.waitFor();

  // Check if the PRO pricing is correctly set by directly using the locator inside the pageSection
...
test(`PRO pricing toggle to Annual price of $${monthlyPrice} -> $${annualPrice}`, async ({ page }) => {
  // Navigate to the pricing page using the base URL from .env
  await page.goto(`${process.env.BASE_URL}/pricing`);
  ...
  // Check if the PRO pricing updates
  const updatedPrice = await page.locator('xpath=/html/body/div[1]/div/main/div[3]/div/div/div/div/div[1]/div/div/div[4]/div[3]/div/div[2]/div/div/div[1]/div[1]/div/span[2]').textContent();
